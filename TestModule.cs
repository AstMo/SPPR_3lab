...
namespace TestModule
{
    public class Task : OSMLSModule
    {
        ...
        public void AddShip()
        {
            ...
            var ship = new Ship(portCoordinate, 15000);
        ...
        public override void Update(long elapsedMilliseconds)
        {
            ...
            if(rand == 1 && storms.Count <= maximumStorms)
            {
                AddStorm();
            }
        ...
        internal void swimToPort(Coordinate coordinates, List<Polygon> storms)
        {
            ...
            if (coordinate.X < coordinates.X)
            {
                ...
                foreach (var storm in storms)
                {
                    ...
                    {
                        var absX = Math.Abs((coordinate.X + Speed) - coordinatesStorm.X);               

                        while (absX < eps)
                        {
                            absX = Math.Abs((coordinate.X + Speed));
                            coordinate.X -= 5 * eps;
                            return;
                        }
            ...
            if (coordinate.Y < coordinates.Y)
            {
                ...
                foreach (var storm in storms)
                {
                    foreach (var coordinatesStorm in storm.Coordinates)
                    {
                        ...
                        var absY = Math.Abs((coordinate.Y + Speed) - coordinatesStorm.Y);

                     
                        while (absY < eps)
                        {
                            absY = Math.Abs((coordinate.Y + Speed));
                            coordinate.Y -= 5 * eps;
                            return;
                        }
                    }
